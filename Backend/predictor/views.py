...
class PredictView(APIView):
    def get(self, request):
        ...
        try:
            result_df = predict_future(symbol, days=days)  # <-- DataFrame

            # ✅ Convert DataFrame into plain dict of lists
            result = {
                "date": result_df["date"].dt.strftime("%Y-%m-%d").tolist(),
                "predicted_close": result_df["predicted_close"].round(2).astype(float).tolist(),
            }

            return Response(result)

        except Exception as e:
            print("Prediction error:", str(e))  # log error in console
            return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
