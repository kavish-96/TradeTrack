...
const PortfolioSummary = () => {
  ...
  const refreshSummary = useCallback(async () => {
    ...
    try {
      ...
      // Fetch positions added in last 24 hours
      const recentPositions = await apiGet('/api/portfolio/positions/recent/');
      let recentPL = 0;
      for (const position of recentPositions) {
        const currentPrice = priceMap[position.symbol] || 0;
        const invested = position.quantity * parseFloat(position.purchase_price);
        const currentValue = position.quantity * currentPrice;
        recentPL += (currentValue - invested);
      }
      setTodayPerformance(recentPL);
      cacheSet('dashboard:portfolio:todayPerformance', recentPL);
    } catch (error) {
